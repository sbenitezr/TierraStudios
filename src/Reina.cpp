...
void Reina::draw(int x)
{
	//PINTADO DE REINA CLASICO
	if (x == 1) {
	...
	//PINTADO DE REINA TEMATICA
	if (x == 2) {
		if (color == color_p::BLANCO) {
			glDisable(GL_LIGHTING);
			glTranslatef(pos.y, -pos.x - 8, 0);
			spritereinaBU.setCenter(0, 0);
			spritereinaBU.setSize(1, 1);
			spritereinaBU.draw();
			glTranslatef(-pos.y, pos.x + 8, 0);
			glEnable(GL_LIGHTING);
		}
		...
		if (color == color_p::NEGRO) {
			glDisable(GL_LIGHTING);
			glTranslatef(pos.y + 1, pos.x, 0);
			spritereinaNU.setCenter(0, 0);
			spritereinaNU.setSize(1, 1);
			spritereinaNU.flip(true, true);
			spritereinaNU.draw();
			glTranslatef(-pos.y - 1, -pos.x, 0);
			glEnable(GL_LIGHTING);
		}
