...
void Alfil::draw(int x)
{
	//PINTADO DE ALFIL MODO CLASICO
	if (x == 1) {
	...
	//PINTADO DE ALFIL MODO TEMATICO
	if (x == 2) {
		if (color == color_p::BLANCO) {
			glDisable(GL_LIGHTING);
			glTranslatef(pos.y, -pos.x - 8, 0);
			spritealfilBU.setCenter(0, 0);
			spritealfilBU.setSize(1, 1);
			spritealfilBU.draw();
			glTranslatef(-pos.y, pos.x + 8, 0);
			glEnable(GL_LIGHTING);
		}
		...
		if (color == color_p::NEGRO) {
			glDisable(GL_LIGHTING);
			glTranslatef(pos.y + 1, pos.x, 0);
			spritealfilNU.setCenter(0, 0);
			spritealfilNU.setSize(1, 1);
			spritealfilNU.flip(true, true);
			spritealfilNU.draw();
			glTranslatef(-pos.y - 1, -pos.x, 0);
			glEnable(GL_LIGHTING);
		}
